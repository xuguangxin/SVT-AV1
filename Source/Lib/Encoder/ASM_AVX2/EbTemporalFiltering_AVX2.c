    ...
    for (int i = 0; i < block_height; i++) {
        for (int j = 0; j < block_width; j++) {
            ...
            const double scaled_diff =
                AOMMAX(-(double)(diff_sse / num_ref_pixels) / (2.0f * h * h), -15.0f);
...
static AOM_FORCE_INLINE void apply_temporal_filter_planewise_hbd(
#if TF_IMP
    ...
    const uint16_t *frame1, const unsigned int stride, const uint16_t *frame2,
    const unsigned int stride2, const int block_width, const int block_height, const double sigma,
    const int decay_control, unsigned int *accumulator, uint16_t *count, uint32_t *luma_sq_error,
...
#else
            const double scaled_diff =
                AOMMAX(-(double)(diff_sse / num_ref_pixels) / (2 * h * h), -15.0f);
            const int adjusted_weight = (int)(exp(scaled_diff) * TF_PLANEWISE_FILTER_WEIGHT_SCALE);
#endif
            count[i * stride2 + j] += adjusted_weight;
            accumulator[i * stride2 + j] += adjusted_weight * pixel_value;
